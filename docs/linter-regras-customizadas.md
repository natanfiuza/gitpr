...
rules:  
  - name: "identificador-da-regra"  
    ...
    message: "🚨 Mensagem de erro que aparecerá no terminal ({file_name}, Linha {line_number})"  
    ignore_comments: true \# Ignora se a regex der match dentro de um comentário (//, \#, /\*)  
...
```YAML
  ...
  - name: "check-route-verbs"  
    ...
    message: "🚨 URI inadequada em {file_name} (Linha {line_number}). Evite verbos como 'buscar' ou 'listar' na URL. Use o padrão RESTful."  
...
YAML
  ...
  - name: "check-php-debug"  
    ...
    message: "🚨 Código de debug esquecido ({file_name}, Linha {line_number})."  
    ignore_comments: true
...
YAML
  ...
  - name: "check-js-console"  
    ...
    message: "🚨 Uso de console.log não permitido em produção ({file_name}, Linha {line_number})."  
    ignore_comments: true
